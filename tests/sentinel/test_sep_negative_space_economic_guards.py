...
def _patch_bridge_sql(monkeypatch, *, splits=None):
    from sentinel.feed import ingest_impl
    ...
    monkeypatch.setattr(
        ingest_impl, "_action_maps",
        lambda *a, **k: ({} if splits is None else splits, {}, [], []))
...
def test_small_published_split_must_survive_canonical_bridge_resolution(monkeypatch):
    ...
    conn = _SequenceConn([
        (100.0, 100.500),
        (dt.date(2026, 4, 3), "AAA", 100.0, 100.503, 0.995),
    ])

    with pytest.raises(
+            guarded.SepNegativeSpaceRefused,
+            match="canonical bridge resolution is fallback"):
...
+    conn = _SequenceConn([
+        (100.0, 101.500),
+        (dt.date(2026, 4, 3), "AAA", 100.0, 100.000, 1.0),
+    ])
+
+    guarded._assert_retirement_preserves_split_chain(conn, _key())
+
+
+def test_derived_only_nonunit_successor_refused_when_bridge_is_unusable(monkeypatch):
+    _patch_bridge_sql(monkeypatch)
+    conn = _SequenceConn([
+        (None, 100.0),
+        (dt.date(2026, 4, 3), "AAA", 50.0, 50.0, 2.0),
+    ])
+
+    with pytest.raises(
+            guarded.SepNegativeSpaceRefused,
+            match="canonical bridge resolution is fallback/1"):
+        guarded._assert_retirement_preserves_split_chain(conn, _key())
+
+
+def test_authoritative_nonunit_successor_survives_unusable_bridge(monkeypatch):
+    _patch_bridge_sql(monkeypatch, splits={
+        ("AAA", "2026-04-03"): 2.0,
+    })
+    conn = _SequenceConn([
+        (None, 100.0),
