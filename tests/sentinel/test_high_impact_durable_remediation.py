...
def test_crash_after_cycle_transition_reconstructs_missing_alert(conn) -> None:
    ...
    assert recovered.event_type == "AUTOMATION_RECOVERED_BLOCKED"
    assert recovered.payload["cycle_id"] == "crash-transition"
    assert recovered.payload["recovered_after_restart"] is True
...
def test_publication_evidence_and_receipt_are_database_enforced(conn) -> None:
    with pytest.raises(publication.CorpusIncoherent, match="JSON object"):
        publication.publish(conn, evidence=[])

    publication._ensure_integrity_schema(conn)
    ...
    with conn.cursor() as cur:
        cur.execute(
            "INSERT INTO sentinel_corpus_publications (evidence)"
            " VALUES ('{}'::jsonb)")
    with pytest.raises(Exception, match="lacks validation receipt"):
        conn.commit()
    conn.rollback()
