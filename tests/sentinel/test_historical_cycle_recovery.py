"""Historical command recovery after the rolling shadow has advanced.
Uses real issuer/source/runtime/certificate fixtures and SQL histories, with
...
import pytest
from sentinel import rolling_runtime, dual_reconciliation
...
def test_new_shadow_close_preserves_old_order_reconciliation(
    ...
    class RecoveryClock(datetime):
        @classmethod
        def now(cls, tz=None):
            return datetime(2026,9,16,14,tzinfo=timezone.utc).astimezone(tz)
    ...
    assert retained.state is (CommandState.FILLED if broker_fill else CommandState.ACKNOWLEDGED)
    assert retained.filled_quantity == Decimal(10 if broker_fill else 0)
...
__all__ = ['conn', 'gateway', 'issuer_source', 'operational_source', 'pg', 'published', 'ready', 'source']
