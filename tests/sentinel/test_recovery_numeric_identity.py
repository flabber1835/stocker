"""Equivalent decimal spellings preserve recovery and position-grace identity."""
...
def test_equivalent_quantity_encoding_preserves_strict_completion(setup,stored,observed,filled):
    ...
    journal.save_command(conn,command)
    result = asyncio.run(observe(conn,bound,now,broker,command,quantity=observed,
        filled=filled,position=filled))
    durable = journal.load_commands(conn,DEPLOYMENT)[0]
    assert durable.quantity == Decimal(10)
