"""config_pin_mismatch — the llm-vetter's side of chain-level config pinning
...
def test_jobs_vet_409_on_stale_pin(monkeypatch):
    """Wiring: /jobs/vet refuses before the vetter-enabled check / any DB read."""
    monkeypatch.setattr(m, "STRATEGY_CONFIG_PATH", V2)
    client = TestClient(app)
    r = client.post("/jobs/vet", params={"expected_config_hash": "deadbeefdeadbeef"})
