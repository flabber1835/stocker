"""config_pin_mismatch — the pipeline's side of chain-level config pinning
...
def test_jobs_run_and_delta_409_on_stale_pin(monkeypatch):
    """Wiring: both pipeline job endpoints refuse before touching the lock/DB."""
    monkeypatch.setattr(m, "STRATEGY_CONFIG_PATH", V2)
    client = TestClient(app)
    for path in ("/jobs/run", "/jobs/delta"):
        r = client.post(path, params={"expected_config_hash": "deadbeefdeadbeef"})
        assert r.status_code == 409, path
        assert r.json()["status"] == "config_mismatch"
    assert not m._job_lock.locked()          # refused BEFORE acquiring the lock
