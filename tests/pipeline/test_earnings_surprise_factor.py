...
def test_beat_scores_higher_than_miss():
    ...
    s = compute_earnings_surprise(pd.DataFrame(rows), date(2026, 6, 27), method="analyst")
...
def test_drift_window_neutralizes_stale_reports():
    ...
    s = compute_earnings_surprise(pd.DataFrame(rows), date(2026, 6, 27), drift_window_days=90, method="analyst")
...
def test_point_in_time_no_lookahead():
    ...
    s_after = compute_earnings_surprise(pd.DataFrame(rows), date(2026, 6, 27), method="analyst")
    s_before = compute_earnings_surprise(pd.DataFrame(rows), date(2026, 6, 15), method="analyst")
...
def test_empty_or_missing_returns_empty():
    assert compute_earnings_surprise(pd.DataFrame(), date(2026, 6, 27), method="analyst").empty
    assert compute_earnings_surprise(None, date(2026, 6, 27), method="analyst").empty
...
def test_fallback_when_too_few_quarters():
    ...
    s = compute_earnings_surprise(pd.DataFrame(rows), date(2026, 6, 27), min_quarters_for_sue=6, method="analyst")
    assert s["NEW"] > 0                          # a beat still registers via fallback


def _srw_history(ticker, latest_eps, latest_date="2026-06-10",
                 latest_fde="2026-03-31"):
    """Same shape as _history but for the SEASONAL-RANDOM-WALK method: fiscal
    periods and reported EPS, no analyst estimate (which Sharadar has none of).

    Year-ago EPS varies slightly so the surprise series has non-zero dispersion —
    a perfectly constant YoY delta is mathematically no surprise at all and
    standardizes to NaN."""
    rows, eps = [], 1.0
    for y in (2023, 2024, 2025):
        for k, (m, d) in enumerate(((3, 31), (6, 30), (9, 30), (12, 31))):
            eps = 1.0 + 0.03 * k + 0.01 * (y - 2023)
            rows.append({"ticker": ticker,
                         "fiscal_date_ending": f"{y}-{m:02d}-{d:02d}",
                         "reported_date": f"{y}-{m:02d}-{d:02d}",
                         "reported_eps": eps, "estimated_eps": None})
    rows.append({"ticker": ticker, "fiscal_date_ending": latest_fde,
                 "reported_date": latest_date, "reported_eps": latest_eps,
                 "estimated_eps": None})
    return rows


def test_factor_ranks_into_0_1_and_beat_tops_miss():
    # End-to-end through compute_all_factors under the DEFAULT method
    # (seasonal_random_walk): the percentiled factor puts the year-over-year
    # grower above the shrinker in [0,1].
    prices = pd.DataFrame([
    ...
    earn = pd.DataFrame(_srw_history("BEAT", 2.5) + _srw_history("MISS", 0.2))
