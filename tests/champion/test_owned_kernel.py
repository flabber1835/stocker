...
def test_complete_kernel_enforces_owned_protection_with_same_core(transitions):
    ...
    first=next(i for i,s in enumerate(stress) if s.last_decision['target_core_exposure']==0.)
    assert first>=4
    # Independent economic severity requirements, not a call to the predicate.
    ...
    # The existing leadership-divergence ceiling can be 0.55 in this all-name
    # decline. It must not be mistaken for the new zero-exposure cause.
    assert all(s.last_decision['target_core_exposure']>0. for s in before[40:])
    assert all(s.last_decision['target_core_exposure']==0. for s in stress[first:])
