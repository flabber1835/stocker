...
def test_saturated_damage_enters_on_fifth_close_without_new_shock():
    ...
    for day in range(5):
        ...
        assert result['target']==(1. if day<4 else .55)
...
def test_entry_is_consecutive_and_rearms_after_recovery():
    ...
    state,r=advance(state,9);assert r['target']==.55
...
def test_recovery_needs_eight_owned_healthy_closes_and_preserves_parent_ceiling():
    ...
    for day in range(5,13):
        state,r=advance(state,day,parent=.65,**healthy())
        assert r['target']==(.55 if day<12 else .65)
...
def test_missing_or_cash_only_book_does_not_clear_protection(missing):
    ...
    assert r['target']==.55 and state['recovery_streak']==0
...
def test_exact_entry_boundaries_are_inclusive():
    state = owned.fresh()
    for day in range(5):
        state, result = advance(state, day, shadow_drawdown=-.10,
                                damaged_breadth=.88, green_breadth=.20)
        assert result['target'] == (1. if day < 4 else .55)
...
def test_new_identity_requires_complete_memory_and_old_identity_rejects_it():
    ...
    old_cfg=load();old_identity=runtime_strategy_identity(old_cfg)
    assert old_cfg.digest!=cfg.digest
    old=SessionState.fresh(starting_cash=50000.,controller=Controller(old_cfg),strategy_identity=old_identity)
...
@pytest.mark.parametrize('parent,expected', [(0.,0.),(.25,.25),(.55,.55),(.65,.55),(1.,.55)])
def test_owned_ceiling_preserves_independent_parent_defense(parent,expected):
    _, result = advance(impaired_state(), 5, parent=parent)
    assert result['target'] == expected


def test_rule_matches_the_frozen_owned55_experiment():
    assert owned.RULE == dict(schema=1, entry_drawdown=-.10, entry_damage=.88, entry_green=.20,
    ...
    assert identity['strategy'] == 'sentinel-compact-champion-owned55-v1'
    assert cfg == owned.load()
