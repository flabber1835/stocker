"""An ephemeral PostgreSQL cluster for tests that need real SQL.

Extracted from the retired `tests/integration/` tier, which was Stocker's
integration harness. The FIXTURE outlived the tier: Sentinel's journal,
reconciliation and restore-drill suites all run real statements against a real
server, because that is the only way their SQL is exercised at all.

Kept as `tests/support/` — TEST-ONLY, and unreachable from any production
dependency path by construction: nothing under `sentinel/` imports `tests`.
"""
