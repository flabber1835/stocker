"""#399 positive F16 acceptance; retained fixture setup, independent payable-share oracle."""
...
def test_dividend_scan_accepts_complete_corpus_for_held_and_unheld_payers(
    ...
    for attempt in range(3):
        with psycopg.connect(conn.info.dsn) as restarted:
            ...
            except trial.TrialEvidenceRefused as error:
                failure = str(error)
                marker = None
            print(f"{history}/{label}: independently_due={oracle}; refusal={failure}; attempt={attempt}")
            assert failure is None, {"required_entitlement":str(oracle), "refusal":failure,
                                     "holdings":holdings,"payer":payer}
            if oracle == 0:
