...
# project, but it cannot start/stop/recreate/remove any resource.
...
if command == "down":
    ...
    for token in tail:
        ...
        if token in forbidden or name in forbidden:
            refuse("destructive Compose down option is not allowed: " + name)
    raise SystemExit(0)
...
if command == "rm":
    ...
    for token in tail:
        name = option_name(token)
        if token in forbidden or name in forbidden:
            refuse("destructive Compose rm option is not allowed: " + name)
    raise SystemExit(0)
