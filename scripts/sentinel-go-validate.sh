...
go_phase() { printf '\n%b=== %s ===%b\n' "$GO_CYAN" "$1" "$GO_RESET"; }
go_info() { printf '%b[GO]%b %s\n' "$GO_GREEN" "$GO_RESET" "$1"; }
go_warn() { printf '%b[WARN]%b %s\n' "$GO_YELLOW" "$GO_RESET" "$1" >&2; }
go_error() { printf '%b[ERROR]%b %s\n' "$GO_RED" "$GO_RESET" "$1" >&2; }
...
for ARG in "$@"; do
  case "$ARG" in
    --input|--input=*|--dev-input) PRODUCTION_RUN=0; FORWARDED_ARGS+=("$ARG") ;;
    --local-full-certification) LOCAL_FULL=1 ;;
    *) FORWARDED_ARGS+=("$ARG") ;;
  esac
done
