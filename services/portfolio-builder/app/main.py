...
async def _do_build(
    ...
    # ── Step 3b: apply universe filters before covariance (smaller matrix = faster) ────────────────────────────
    t0 = datetime.now(timezone.utc)
    universe_cfg = strategy.universe
    min_price = universe_cfg.min_price
    min_avg_dv = universe_cfg.min_avg_dollar_volume_20d

    # Price filter: use already-loaded prices_df (no extra DB round-trip needed)
    latest_price_map = (
    ...
    # Avg dollar-volume filter: query fundamentals table
    async with engine.connect() as conn:
    ...
    price_filtered = [t for t in rankable_tickers if latest_price_map.get(t, 0) < min_price]
    ...
    if not filtered_tickers:
        raise RuntimeError("no candidates passed universe filters (min_price / min_avg_dollar_volume)")
    ...
    async with engine.begin() as conn:
        await _log_step(
            ...
            input_summary={
                ...
                "ticker_count_in": len(rankable_tickers),
            },
            output_summary={
            ...
            warnings=(
                [f"{len(universe_filtered)} tickers filtered: price<{min_price} or avg_dv<{min_avg_dv} or no_avg_dv_data"]
                if universe_filtered else None
            ),
        )

    # ── Step 4: build covariance matrix on filtered set ─────────────────────────────────────────────────────────
    t0 = datetime.now(timezone.utc)
        ...
        for item in selected:
            ...
            await conn.execute(
                text(
                    ...
                    "    portfolio_vol_at_add=EXCLUDED.portfolio_vol_at_add, "
...
async def _assert_no_running_job() -> None:
    async with engine.connect() as conn:
        ...
        if row.fetchone() is not None:
            raise HTTPException(
                status_code=409,
                detail="Another portfolio build job is already running. Wait for it to complete.",
            )
