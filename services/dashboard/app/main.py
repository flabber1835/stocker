...
<div id="pane-universe" class="pane">
  ...
  <div class="toolbar">
    ...
    <button class="btn" id="u-hide-tiny" onclick="toggleTiny()" title="Hide stocks with weight &lt; 0.01%">HIDE TINY &#10003;</button>
...
function renderUniverse(){
  ...
  let rows=uniData.filter(t=>{
    if(!q||t.ticker.includes(q)||(t.name||'').toUpperCase().includes(q)){
      if(!sec||t.sector===sec){
        if(uniHideTiny&&(t.weight_pct==null||+t.weight_pct<0.01))return false;
        return true;
      }
    }
    return false;
  ...
  $('u-body').innerHTML=rows.map(t=>{
    const tiny=t.weight_pct!=null&&+t.weight_pct>0&&+t.weight_pct<0.01;
    const rowStyle=tiny?'opacity:0.5':'';
