...
from app.factor_inputs import (
    FUND_FIELDS,
    FUND_LKG_WINDOW_DAYS,
    MARKET_BENCHMARK,
    REGIME_UNKNOWN as _REGIME_UNKNOWN,
    FactorInputs,
    load_factor_inputs,
    lkg_fundamentals_sql as _lkg_fundamentals_sql,
)
...
# FUND_LKG_WINDOW_DAYS / FUND_FIELDS / _lkg_fundamentals_sql live in
# app.factor_inputs (imported above) — the loader that consumes them.
...
# MARKET_BENCHMARK and _REGIME_UNKNOWN live in app.factor_inputs (imported above).
...
async def _do_calculate(run_id: str, trace_id: str, today: date, started_at: datetime) -> date:
    ...
    """
    async def _log(step_name, status, **kw):
        async with engine.begin() as conn:
            await _log_step_factor(conn, trace_id, step_name, status, **kw)

    inputs = await load_factor_inputs(
        engine, strategy,
        log=_log,
        progress=lambda pct: _set_pct("calc_factors", pct),
    )
    universe_tickers = inputs.universe_tickers
    sector_map = inputs.sector_map
    score_date = inputs.score_date
    raw_regime = inputs.raw_regime
    confirmed_regime = inputs.confirmed_regime
    regime_info = inputs.regime_info
    prices_df = inputs.prices_df
    fund_df = inputs.fundamentals_df
    earnings_df = inputs.earnings_df
    snapshot_id = inputs.snapshot_id
    _st = inputs.stats
    coverage_by_ticker = _st["coverage_by_ticker"]
    tickers_with_prices = _st["tickers_with_prices"]
    tickers_with_fundamentals = _st["tickers_with_fundamentals"]
    fund_etf_dropped = _st["fund_etf_dropped"]
    price_max_date = _st["price_max_date"]
...
# ── Factor-recompute preview (read-only) ─────────────────────────────────────
#
# Deliberately NOT under /jobs/ — every path there persists a run row. This one
# writes NOTHING. It exists because the evaluator can only triage config changes
# that RE-WEIGHT persisted factor scores (preview_ranking, config_replay); a
# change to how a factor is COMPUTED (momentum_blend_windows, momentum_method,
# volatility_window, pe_pb_cap, sue_method) had exactly one evaluable route, a
# multi-hour wind-tunnel run. One such candidate burned 3h31m and a full
# experiment-lane slot to come back at -1.18pp.
#
# It answers ONE question — "does this factor_engine change reorder the book?" —
# for ONE date. It is a screen, not evidence: no returns, no equity curve, and
# deliberately no backtest_trials registration, because a rank diff is not a
# performance estimate and must not deflate anyone's DSR.

PREVIEW_LOCK_TIMEOUT_SECS = float(os.getenv("PREVIEW_LOCK_TIMEOUT_SECS", "5"))


def _spearman(a: "pd.Series", b: "pd.Series") -> float | None:
    """Rank correlation without scipy (the pipeline image has no scipy)."""
    import numpy as np
...
@app.post("/preview/factors")
async def preview_factors(payload: dict):
    """Recompute factors for the latest scored date under a CANDIDATE config and
    return both rankings. Persists nothing.

    Body: {"config": <full StrategyConfig dict>}.
    """
    candidate_dict = (payload or {}).get("config")
    ...
    try:
        candidate = StrategyConfig(**candidate_dict)
    except Exception as exc:
        raise HTTPException(status_code=400, detail=f"candidate config INVALID: {exc}")

    # The universe block is out of scope BY CONSTRUCTION, not by omission: this
    # preview reuses the surviving tickers of the run it diffs against, precisely
    # so the candidate is scored over the same population as the active ranking.
    # Honour a universe change here and that guarantee is gone — so refuse, and
    # name the rig that CAN model it rather than silently scoring it as inert.
    if candidate.universe.model_dump() != active.universe.model_dump():
        raise HTTPException(
            status_code=422,
            detail="this preview cannot model a `universe.*` change (it reuses the "
                   "investable set of the run it diffs against, which is what makes "
                   "the comparison apples-to-apples). Score it in the wind tunnel.",
        )
    ...
    async with engine.connect() as conn:
        ...
        if not run_row:
            raise HTTPException(status_code=409, detail="no successful factor run to preview against")
        tickers = [r[0] for r in (await conn.execute(text(
    ...
    # Serialize against the chain. This is a memory guard, not a correctness one:
    # the preview holds a universe-scale price frame, and the pipeline runs under a
    # hard mem_limit that the factor step already sits close to.
    try:
    ...
    try:
        # Load enough history for BOTH configs. `price_lookback` derives from
        # max(momentum_long_window, volatility_window); loading against the ACTIVE
        # config alone would hand a candidate that LENGTHENS either window a frame
        # too short for it, nulling its factor — which would surface as a coverage
        # collapse, the single artifact the prompt tells the model to trust most.
        # `momentum_blend_windows` is deliberately NOT widened: live's own lookback
        # formula ignores it, so a blend window past momentum_long_window starves in
        # production too, and the preview should report what live would actually do.
        loader_cfg = active
        _a, _c = active.factor_engine, candidate.factor_engine
        _widen = {k: max(getattr(_a, k), getattr(_c, k))
                  for k in ("momentum_long_window", "volatility_window",
                            "spy_price_lookback_days")
                  if getattr(_c, k) > getattr(_a, k)}
        if _widen:
            loader_cfg = active.model_copy(
                update={"factor_engine": _a.model_copy(update=_widen)})
        inputs = await load_factor_inputs(engine, loader_cfg, universe_override=tickers)
        regime = inputs.confirmed_regime

        same_engine = candidate.factor_engine.model_dump() == active.factor_engine.model_dump()

        def _compute(cfg):
            # copy_input=False on BOTH calls. Safe because the mutations
            # compute_all_factors performs on the frame — to_datetime on datetimes,
            # an in-place sort of an already-sorted frame, astype(float) on floats —
            # are all IDEMPOTENT. So the second compute costs no extra peak memory,
            # which is what makes recomputing our own baseline affordable.
            return compute_all_factors(
                prices_long=inputs.prices_df,
                fundamentals=inputs.fundamentals_df.drop(columns=["as_of_date"], errors="ignore"),
                cfg=cfg,
                copy_input=False,
                sector_map=inputs.sector_map,
                earnings=inputs.earnings_df,
                as_of_date=inputs.score_date,
            )

        def _work():
            # Recompute the ACTIVE side too rather than reading persisted scores: a
            # persisted run may carry a different price vintage or universe snapshot,
            # and comparing recompute-against-persisted would mix a data diff into
            # the config diff. `baseline_fidelity` below reports how far the two are
            # apart so that assumption is visible, not assumed.
            af = _compute(active.factor_engine)
            cf = af if same_engine else _compute(candidate.factor_engine)
            return af, cf

        active_f, cand_f = await asyncio.to_thread(_work)
        active_ranked = await asyncio.to_thread(rank_universe, active_f, regime, active)
        cand_ranked = await asyncio.to_thread(rank_universe, cand_f, regime, candidate)
    finally:
        if _job_lock.locked():
            _job_lock.release()

    coverage, corr = {}, {}
    # Align on ticker rather than position: both frames are built from the same
    # `prices_long["ticker"].unique()` today, but a per-ticker correlation silently
    # comparing two different names would be undetectable in the output.
    _a_idx = active_f.set_index("ticker")
    _c_idx = cand_f.set_index("ticker")
    for f in FACTORS:
        if f in _a_idx.columns and f in _c_idx.columns:
            coverage[f] = {
                "active_non_null": int(_a_idx[f].notna().sum()),
                "candidate_non_null": int(_c_idx[f].notna().sum()),
            }
            corr[f] = None if same_engine else _spearman(_a_idx[f], _c_idx[f])

    # baseline_fidelity: our recomputed active ranking vs the one actually
    # persisted for this date. Below ~1.0 means the baseline this diff is measured
    # against has drifted from what the chain really produced — every conclusion
    # off it is contaminated, and the caller must be able to SEE that rather than
    # trust it. Reported, never asserted.
    baseline_fidelity = None
    try:
        async with engine.connect() as conn:
            persisted = {r[0]: r[1] for r in (await conn.execute(text(
                "SELECT ticker, rank FROM rankings WHERE run_id = ("
                "  SELECT run_id FROM ranking_runs WHERE status='success' "
                "    AND source_factor_run_id = :fid "
                "  ORDER BY completed_at DESC NULLS LAST LIMIT 1)"
            ), {"fid": run_row["run_id"]})).fetchall()}
        if persisted:
            mine = active_ranked.set_index("ticker")["rank"]
            theirs = pd.Series(persisted, dtype=float)
            baseline_fidelity = _spearman(mine.astype(float), theirs)
    except Exception as exc:
        print(f"[preview] baseline_fidelity unavailable: {exc}", flush=True)

    def _rows(df):
        return [{"ticker": r["ticker"], "rank": int(r["rank"]),
                 "composite_score": (None if pd.isna(r["composite_score"])
                                     else round(float(r["composite_score"]), 6))}
                for _, r in df.iterrows()]

    return {
        "score_date": str(inputs.score_date),
        "regime": regime,
        "n_tickers": len(inputs.universe_tickers),
        "active_config_hash": active_hash,
        "factor_engine_changed": not same_engine,
        "active_ranking": _rows(active_ranked),
        "candidate_ranking": _rows(cand_ranked),
        "factor_coverage": coverage,
        "factor_rank_correlation": corr,
