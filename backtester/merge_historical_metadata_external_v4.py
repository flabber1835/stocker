...
def merge_external(inputs: Path, output: Path, *, expected_shards: int, expected_cohort: int,
    ...
    for shard in range(expected_shards):
        ...
        verify_checksums(plan_root)
        verify_checksums(evidence_root)
        ...
        pids = {row["security_id"] for row in shard_plans}
        rids = {row["security_id"] for row in shard_results}
        ...
        for row in shard_evidence:
            ...
            source_path = evidence_root / member
        ...
        manifests.extend(shard_manifest)
        candidate_status.update({
            "candidate_found": int(es.get("candidate_found", 0)),
            "ambiguous": int(es.get("ambiguous", 0)),
            "no_discovery_match": int(es.get("no_discovery_match", 0)),
            "no_archived_proof": int(es.get("no_archived_proof", 0)),
        })
    ...
    plans.sort(key=lambda r: (r["ticker"], r["security_id"]))
    results.sort(key=lambda r: (r["ticker"], r["security_id"]))
    ...
    write_csv_gz(output / "plan.csv.gz", PLAN_FIELDS, plans)
    write_csv_gz(output / "candidate_evidence.csv.gz", EXTERNAL_EVIDENCE_FIELDS, evidence)
    write_csv_gz(output / "episode_results.csv.gz", RESULT_FIELDS, results)
    write_csv_gz(output / "source_manifest.csv.gz", MANIFEST_FIELDS, manifests)
    summary = {
        "schema": EXTERNAL_MERGE_SCHEMA,
        "status": "PASS",
        "candidate_only": True,
        ...
        "episodes": len(plan_ids),
        "candidate_evidence_rows": len(evidence),
        "source_manifest_rows": len(manifests),
        "unique_frozen_source_objects": len(source_index),
        "new_source_files_copied": copied,
        "source_inventory_sha256": inventory_sha,
        "episode_status": dict(candidate_status),
...
def _normalized_identity_quality(form: str, proof: str) -> str:
    ...
    if proof == "SEC_OWNERSHIP_ISSUER_TRADING_SYMBOL_XML":
        return proof
    if proof in {"SEC_EXPLICIT_TRADING_SYMBOL_LABEL", "SEC_EXCHANGE_QUALIFIED_TICKER"}:
...
def _type_quality(form: str) -> str:
    ...
    if form in CURRENT_FORMS:
        return "CURRENT_FORM_EXACT_TICKER_CLASS_CANDIDATE"
    if form in EXTENDED_FORMS:
        return "EXTENDED_FORM_EXACT_TICKER_CLASS_CANDIDATE"
    if form in OWNERSHIP_FORMS:
        return "OWNERSHIP_FORM_SUPPLEMENTARY_CLASS_ONLY"
...
def combine(retained_root: Path, external_root: Path, output: Path, *, expected_retained_sha256: str) -> dict[str, object]:
    ...
    for row in external_rows:
        ...
        if not plan:
            raise RuntimeError("external candidate lacks merged plan episode")
        ...
        base = {
            ...
            "resolution_route": "EXTERNAL_SEC_STRICT_PRIOR",
            "candidate_cik": row.get("candidate_cik", ""),
            "cik_authority": "DISCOVERY_ONLY_HINT",
            ...
            "artifact_member": row.get("source_member", ""),
        ...
        if classification in {"common", "non_common"}:
            transformed.append(base | {
                ...
                "candidate_quality": _type_quality(form),
                "classification": classification,
                "sic": "",
                "evidence_excerpt": row.get("classification_excerpt", ""),
            })
        ...
        if 3 <= len(sic) <= 4:
            ...
            transformed.append(base | {
                ...
                "candidate_quality": "HEADER_SIC_SAME_FILING_EXACT_TICKER_BOOTSTRAP",
                "classification": "",
                "sic": sic,
    ...
    summary = {
        "schema": OWNERSHIP_STRICT_MERGE_SCHEMA,
        "status": "PASS",
        "candidate_only": True,
        "admission_effect": "NONE",
        "merged_shards": 32,
        ...
        "retained_candidate_sha256": retained_sha,
        "external_authority_extension": True,
        "external_authority_shards": external_summary["merged_shards"],
        "external_authority_episodes": external_summary["episodes"],
        "external_evidence_rows": len(external_rows),
        "external_transformed_candidate_rows": len(transformed),
        "external_source_run_id": external_summary["source_run_id"],
        "external_source_sha": external_summary["source_sha"],
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    p = sub.add_parser("merge-external")
    p.add_argument("--inputs", type=Path, required=True)
    p.add_argument("--output", type=Path, required=True)
    p.add_argument("--expected-shards", type=int, default=8)
    p.add_argument("--expected-cohort", type=int, default=4037)
    p.add_argument("--source-run-id", required=True)
    p.add_argument("--source-sha", required=True)
    p = sub.add_parser("combine")
    p.add_argument("--retained-root", type=Path, required=True)
    p.add_argument("--external-root", type=Path, required=True)
    p.add_argument("--output", type=Path, required=True)
    p.add_argument("--expected-retained-sha256", default="")
    args = parser.parse_args(argv)
    if args.cmd == "merge-external":
        result = merge_external(args.inputs, args.output, expected_shards=args.expected_shards,
                                expected_cohort=args.expected_cohort, source_run_id=args.source_run_id,
                                source_sha=args.source_sha)
    else:
        result = combine(args.retained_root, args.external_root, args.output,
                         expected_retained_sha256=args.expected_retained_sha256)
