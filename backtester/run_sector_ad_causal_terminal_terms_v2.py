...
A is the exact current-main/current-Sharadar-metadata control.
D is the retained PIT-correct economic variant. Its only economically active
metadata delta is strict-prior SEC SIC -> FF12 sector grouping. Category removal,
SEC CIK issuer-family authority, and exchange treatment were already proven
zero-delta on the retained economic path, so this launcher preserves exact
Wealth Core parity and applies the active PIT delta at Sentinel's grouping seam.

The strategy implementation remains the exact pinned ``main`` checkout.
"""
...
spec = importlib.util.spec_from_file_location("sector_ad_base_runner_v2", BASE_RUNNER)
if spec is None or spec.loader is None:
    raise RuntimeError(f"cannot load base runner from {BASE_RUNNER}")
runner = importlib.util.module_from_spec(spec)
spec.loader.exec_module(runner)
runner.EXPERIMENT_ID = "2026-08-27-sector-ad-v2"
...
def _load_exact_terms(resolver) -> dict[str, tuple[object, ...]]:
    ...
    for record in payload.get("records") or []:
        ...
        expected_source_hash = runner.source_hash_for_year(phase1, int(witness_session[:4]))
...
def _terminal_events_with_exact_terms(session: str, rows, priced_tickers, resolver, main):
    vendor_events = _real_build_terminal_events(session, rows, priced_tickers, resolver, main)
    ...
    if str(self.name) == "B":
        ...
        if _progress_sessions % PROGRESS_INTERVAL == 0 or final:
            ...
            print(
                ...
                f"A_multiple={a_nav:.10f} A_cagr={_running_cagr(a_nav, session):.10%} "
                f"D_multiple={d_nav:.10f} D_cagr={_running_cagr(d_nav, session):.10%}",
                flush=True)
    return nav
...
def _postprocess_ad_bundle() -> None:
    ...
    daily.rename(
        columns={column: "D_" + column[2:] for column in daily.columns if column.startswith("B_")},
        inplace=True)
    daily.to_csv(
    ...
    summary["variant_definition"] = {
        "A": "exact current-main current-Sharadar-metadata control",
        "D": (
            "retained PIT-correct economic equivalent: strict-prior SEC SIC -> frozen FF12 "
            "at Sentinel grouping seam; category removal and SEC CIK issuer authority are "
            "retained zero-delta equivalence proofs; exchange is inert"
        ),
    }
    ...
    summary["d_pit_semantics"] = {
        "category": "equivalence-proven zero economic delta; no duplicate mutation in this replay",
        "exchange": "inert/non-authoritative",
        "issuer_family": "SEC CIK authority equivalence-proven zero economic delta on retained path",
        "sector": "strict-prior SEC SIC -> frozen FF12",
        "missing_sector": "singleton unknown peer",
        "wealth_core_path": "exact A/D parity required every session",
    }
...
def main() -> int:
    print(f"[RUN] A/D cumulative CAGR checkpoint interval={PROGRESS_INTERVAL} sessions", flush=True)
    rc = int(runner.main())
    if rc != 0:
        return rc
    _postprocess_ad_bundle()
    _postprocess_provenance()
    print("[PASS] A/D PIT-equivalent replay and causal terminal provenance recorded", flush=True)
    return 0
