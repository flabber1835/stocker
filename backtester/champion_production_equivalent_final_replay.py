...
from backtester.champion_economic_prefix_audit import EXPECTED_CORPUS, PROFILE, PROFILE_HASH, RUNTIME, SOURCE
...
def main():
    ...
    manifest=json.loads((Path(__import__('os').environ['CANONICAL_PIT_DATASET'])/'manifest.json').read_text())
    ...
    report={'schema':'champion.production-equivalent-certificate/1','status':'PRODUCTION_EQUIVALENT_CERTIFIED',
            ...
            'generated_source_sha256':sha(final.encode()),'windows':windows}
