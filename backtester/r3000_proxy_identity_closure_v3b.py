#!/usr/bin/env python3
"""Stage B3b: optimized offline SEC-name closure.

Semantics are identical to B3a, but canonical observations are loaded only for
sessions actually needed by unresolved SEC rows and pre-existing duplicate identity
groups. This avoids scanning irrelevant yearly partitions.
"""
...
SCHEMA = "stocker.r3000-proxy.identity-closure/3b"
B2_SCHEMA = "stocker.r3000-proxy.identity-closure/2"


def close_identity_v3b(b2_root: Path, canonical_root: Path, output: Path) -> dict:
    ...
    if b2_summary.get("schema") != B2_SCHEMA or b2_summary.get("status") != "PASS":
        raise RuntimeError("B2 artifact is not a passing schema-2 package")
    fields, rows = read_csv(b2_ledger_path)
    if len(rows) != int(b2_summary.get("source_rows", -1)):
        raise RuntimeError("B2 row count changed")
    ...
    all_dates = {row["holdings_effective_date"] for row in rows}
    all_targets = _target_sessions(sessions, all_dates)
    for row in rows:
        if row.get("identity_target_session") != all_targets[row["holdings_effective_date"]]:
    ...
    assignments_before: dict[tuple[str, str, str], list[int]] = defaultdict(list)
    for index, row in enumerate(rows):
        sid = row.get("normalized_security_id", "")
        if row.get("identity_status") == "RESOLVED" and sid:
            assignments_before[(row.get("fund", ""), row.get("holdings_effective_date", ""), sid)].append(index)
    duplicate_groups_before = {key: indexes for key, indexes in assignments_before.items() if len(indexes) > 1}

    needed_dates = {
        row["holdings_effective_date"]
        for row in rows
        if row.get("source_type") == "sec_n-q" and row.get("identity_status") != "RESOLVED"
    }
    needed_dates.update(key[1] for key in duplicate_groups_before)
    needed_targets = {date: all_targets[date] for date in sorted(needed_dates)}
    by_price, _ = _load_target_prices(canonical_root, needed_targets)
    ...
    for row in rows:
        ...
        if price is None or not candidates:
            ...
            sec_no_price += 1
            continue
        variants = _name_variants(row.get("reported_issuer_name", ""))
        matched: set[str] = set()
        for variant in variants:
            matched.update(name_index.get(variant, set()))
        matched &= set(candidates)
        detail = {
            "method": "OFFLINE_SEC_NORMALIZED_NAME_PLUS_HISTORICAL_PRICE",
            "source_name": row.get("reported_issuer_name", ""),
            "source_name_variants": sorted(variants),
            "historical_price_cents": format(price, "f"),
            "canonical_dataset_hash": dataset_hash,
        }
        if len(matched) == 1:
            sid = next(iter(matched))
            _set_resolved(row, candidates[sid], method="OFFLINE_SEC_NORMALIZED_NAME_PLUS_HISTORICAL_PRICE", reason="", detail=detail)
            sec_resolved += 1
        elif len(matched) > 1:
            detail["candidate_security_ids"] = sorted(matched)
            _set_open(row, status="AMBIGUOUS", method="OFFLINE_SEC_NORMALIZED_NAME_PLUS_HISTORICAL_PRICE", reason="MULTIPLE_EXACT_NAME_AND_PRICE_MATCHES", candidate_count=len(matched), detail=detail)
            sec_ambiguous += 1
        else:
            row["identity_method"] = "OFFLINE_SEC_NORMALIZED_NAME_PLUS_HISTORICAL_PRICE"
    ...
    # Recompute after SEC promotions, then fail closed on every duplicate SID.
    ...
    for (fund, effective, sid), indexes in sorted(assignments.items()):
        ...
        for index in indexes:
            _set_open(rows[index], status="AMBIGUOUS", method="DUPLICATE_SECURITY_ID_FAIL_CLOSED", reason="SAME_FUND_SNAPSHOT_SECURITY_ID_COLLISION_REQUIRES_INDEPENDENT_ISSUER_AUTHORITY", candidate_count=len(indexes), detail={
    ...
    ledger_path = output / "identity_ledger_v3b.csv.gz"
    worklist_path = output / "identity_unresolved_worklist_v3b.csv.gz"
    write_csv_gz(ledger_path, fields, rows)
    write_csv_gz(worklist_path, fields, [row for row in rows if row.get("identity_status") != "RESOLVED"])
    snapshot_rows = []
    for (fund, effective), snap_counts in sorted(snapshot_counts.items()):
        snapshot_rows.append({
            "fund": fund,
            "holdings_effective_date": effective,
            "identity_target_session": all_targets[effective],
            "rows": str(sum(snap_counts.values())),
            **{status.lower(): str(snap_counts.get(status, 0)) for status in STATUSES},
        })
    write_csv_gz(output / "identity_snapshot_summary_v3b.csv.gz", ["fund", "holdings_effective_date", "identity_target_session", "rows", "resolved", "ambiguous", "unmatched", "conflict"], snapshot_rows)

    summary = {
        "schema": SCHEMA,
        "stage": "IDENTITY_CLOSURE_OFFLINE_SEC_NAME_PASS_3B",
        ...
        "canonical_partitions_loaded": sorted({int(date[:4]) for date in needed_dates}),
        ...
        "methods_added": {
            "sec_normalized_name_plus_historical_price": sec_resolved,
        ...
        "historical_state_semantics": "Every new B3b identity requires exact deterministic source-name normalization, continuity to a B2 price-certified BlackRock identity, and independent historical source-price agreement on the canonical target session.",
    ...
    summary_path = output / "identity_summary_v3b.json"
...
def main() -> int:
    ...
    print(json.dumps(close_identity_v3b(args.b2, args.canonical, args.output), indent=2, sort_keys=True))
    return 0
