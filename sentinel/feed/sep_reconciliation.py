...
def _source_fingerprint(conn, *, fetch, start: str, end: str, observation_ceiling):
    ...
    return _core._source_fingerprint(conn, fetch=guarded, start=start, end=end)
...
def _complete_export_source(*, start: str, end: str):
    ...
    try:
        for lo, hi in _month_windows(start, end):
            ...
            if refresh_marker is None:
                ...
                raise SepReconciliationStateInvalid(
                    "SEP complete export composite crossed a vendor table refresh; "
                    "refusing mixed-generation deletion authority")
    ...
    def export_fetch(table, params=None, **_kwargs):
        if table != sharadar.SEP or dict(params or {}) != expected:
            raise ValueError(
                "SEP retirement export may be replayed only for its exact bounded partition")
        def replay():
...
def _fresh_actions_retirement_authority(conn, *, through: dt.date) -> dict:
    ...
    maintenance.reconcile_actions_if_due(conn, through=through.isoformat(), force=True)
    rows, evidence = snapshot_export.fetch_complete_actions(through=through.isoformat())
    source = {identity: payload for identity, payload, _row in action_source.distinct_rows(rows)}
    active = maintenance._active_action_rows(conn)
    local = {identity: action_source.canonical_payload(row) for identity, row in active.items()}
    if source != local:
        missing = sorted(set(source) - set(local))
        extra = sorted(set(local) - set(source))
        changed = sorted(identity for identity in set(source).intersection(local)
    ...
    bound = dict(evidence)
    bound.update({"verified_through": through.isoformat(),
                  "verified_publication_version": int(current.version),
                  "verified_distinct_rows": len(source)})
    return bound


def _repair_local_only_if_proved(conn, *, fetch, start: str, end: str,
                                  observation_ceiling, source, local,
    ...
    if require_complete_export:
        if actions_authority_evidence is None:
            ...
            if int(local.rows) <= int(source.rows):
                if (local.rows == source.rows and local.key_digest == source.key_digest
        ...
        repair_fetch, source_authority_evidence = _complete_export_source(start=start, end=end)
    try:
...
def reconcile_year(conn, *, fetch=None, year: int, start: str, end: str,
    ...
    if not (str(start).startswith(f"{int(year):04d}-") and str(end).startswith(f"{int(year):04d}-")):
    ...
    if source.rows != local.rows or source.key_digest != local.key_digest:
        local = _repair_local_only_if_proved(
            ...
            observation_ceiling=observation_ceiling, source=source, local=local,
            require_complete_export=bool(require_complete_export),
            actions_authority_evidence=actions_authority_evidence)
    if source.rows != local.rows or source.key_digest != local.key_digest:
        raise _core.SepKeysetDrift(
            f"stable Sharadar SEP {year} normalized key set disagrees with published corpus: "
            f"source {source.rows:,}/{source.key_digest[:16]}, local {local.rows:,}/{local.key_digest[:16]}. "
            "This can be a vendor insertion, identity restatement, lost local row, or an unproved "
            "deletion outside the bounded negative-space contract. Refusing to guess which side to repair.")
    if source.value_digest != local.value_digest:
        raise _core.SepValueDrift(
            f"stable Sharadar SEP {year} strategy values disagree with published corpus despite an identical "
            f"{source.rows:,}-row key set: source {source.value_digest[:16]}, local {local.value_digest[:16]}. "
            "At least one signal/raw/open/volume value is stale or corrupted; refusing to earn/advance reconciliation authority over it.")
    current = _core.publication.require_current(conn)
    return _core.ReconciliationResult(
        ...
        max_lastupdated=source.max_lastupdated, publication_version=current.version)
...
def reconcile_all(conn, *, fetch=None, through: str, observation_ceiling=None):
    ...
    source_ceiling = (_strict_ceiling(observation_ceiling) if observation_ceiling is not None
                      else _production_source_ceiling(fetch, market_through))
    if source_ceiling < market_through:
        raise SepReconciliationStateInvalid(
            f"current source observation date {source_ceiling} is behind market reconciliation boundary {market_through}")
    require_complete_export = production or fetch is snapshot_source.fetch_table
    if require_complete_export:
        maintenance.reconcile_actions_if_due(conn, through=market_through.isoformat())
        maintenance.reconcile_sep_mutations(conn, fetch=fetch,
                                            through=source_ceiling.isoformat(), reobserve_equal=True)
    lo, hi = _visible_bounds(conn)
    results = []
    for year, start, end in _bounded_years(lo, hi, market_through):
        result = reconcile_year(conn, fetch=fetch, year=year,
