    ...
    ON CONFLICT (permaticker,ticker) DO UPDATE SET
      category=CASE
        WHEN EXCLUDED.snapshot_date >= feed_universe_current.snapshot_date
          THEN COALESCE(EXCLUDED.category,feed_universe_current.category)
        ELSE feed_universe_current.category END,
      sector=CASE
        WHEN EXCLUDED.snapshot_date >= feed_universe_current.snapshot_date
          THEN COALESCE(EXCLUDED.sector,feed_universe_current.sector)
        ELSE feed_universe_current.sector END,
      related_tickers=CASE
        WHEN EXCLUDED.snapshot_date >= feed_universe_current.snapshot_date
          THEN COALESCE(EXCLUDED.related_tickers,
                        feed_universe_current.related_tickers)
        ELSE feed_universe_current.related_tickers END,
      ...
      is_delisted=CASE
        WHEN EXCLUDED.snapshot_date >= feed_universe_current.snapshot_date
          THEN COALESCE(EXCLUDED.is_delisted,
                        feed_universe_current.is_delisted)
