...
def apply_supported_entitlements(state, distributions, *, bars, ledger,
    ...
    ambiguous_bars = set()
    for item in bars:
        if item.security_id in bar_by_security:
            ambiguous_bars.add(item.security_id)
        bar_by_security[item.security_id] = item
    plans = []
    seen = set()
    for event in distributions:
        ...
        if (event.parent_security_id in ambiguous_bars
            ...
            raise SpinoffTermsRequired("ambiguous held spin-off opening bars")
        ...
        if (parent_bar.ticker.upper() != event.parent_ticker.upper()
            ...
            raise SpinoffTermsRequired("held spin-off opening bar identity conflicts")
        ...
        plans.append({
            ...
            "per_parent_child_value": float(ratio * Fraction(str(child_open))),
        })
    ...
    for plan in plans:
        event = plan["event"]
        group = groups.setdefault((event.session, event.parent_security_id), {
            "parents": plan["parents"], "parent_open": plan["parent_open"],
            "child_values": [],
        })
        group["child_values"].append(plan["per_parent_child_value"])
    for group in groups.values():
        # Aggregate gross child value before rebasing once. A product of
        # individual child scales would reduce the retained basis too far.
        scale = group["parent_open"] / (
            group["parent_open"] + math.fsum(group["child_values"]))
    ...
    for plan in plans:
        event = plan["event"]
        reference_scale = groups[(event.session, event.parent_security_id)]["reference_scale"]
        before = state.cash
        ledger.post(
