...
Construction per decision close t:

1. Start from the same causally available Wealth Core eligible population.
2. Leadership population size is max(25, ceil(10% * eligible_count)).
3. Established leaders rank by raw 6-to-1 momentum (close[t-21]/close[t-126]-1).
4. Recent leaders use the same population size and rank by raw latest 21-session
   price return (close[t]/close[t-21]-1).
5. Deterministic tie-break is security identity ascending after score descending.
6. Membership selected at close t earns the next close-to-close *price* return
   t->t+1.  No dividend cash is added to this shadow.
7. The recent shadow is arithmetic equal-weight.  A selected constituent that
   has no valid next-session price print contributes 0% for that session rather
   than being dropped and reweighted.
8. recent_r20/r40 are session-count returns of the resulting shadow NAV.

Economic eligibility upstream must use corrected Sharadar liquidity
``SEP.close * SEP.volume`` and historical/as-traded price gate semantics.  This
module deliberately does not duplicate Wealth Core eligibility logic; callers
must supply the canonical eligible rows for one close.

Retained population-construction fingerprints:
  2008-12-23 overlap = 7/101 = 6.9306930693%
  2022-01-03 overlap = 8/96  = 8.3333333333%
"""
...
def equal_weight_next_close_return(
    ...
    """Return the recovered arithmetic equal-weight witness return.

    Membership was selected at the previous close.  Every selected name keeps
    its 1/N weight for this one-session return.  Missing/invalid current or
    previous price evidence contributes 0%, matching the retained research
    ``recent_nav_zero`` convention rather than dropping and reweighting names.
    """
    ...
    for security_id in ids:
        ...
        # Else: fixed constituent weight earns exactly 0% for this session.
