...
def fresh(*, champion=False):
    return {"version": 2 if champion else 1,
            **({"champion_audit": {"episodes": 0, "concordance_releases": 0}} if champion else {}),
            "episode": False, "latched": False,
...
def validate(raw):
    ...
    champion = raw["version"] == 2
    if raw["version"] not in (1, 2) or set(raw) != set(fresh(champion=champion)):
        raise ValueError("invalid Median-5 controller state schema")
    ...
    if champion:
        from .champion import validate_recovery
