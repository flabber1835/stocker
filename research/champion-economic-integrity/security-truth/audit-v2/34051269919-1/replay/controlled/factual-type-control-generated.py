...
gc.disable()
ROOT = Path('sharadar')
OUT = Path('/home/runner/work/stocker/stocker/factual-replay/controlled/engine')
...
def run():
    ...
    for y in range(2006,END.year+1):
        ...
        for date,g in d.groupby('date',sort=True):
            ...
            _bestclass.observe(ds,tuple(str(sid[int(x)]) for x in durable),tuple(str(sid[int(s.tid)]) for s in book.slots if s.held()),tuple(str(sid[int(s.pending_tid)]) for s in book.slots if s.reserved()),tuple(str(sid[int(x)]) for x in recsel),tuple(str(sid[int(x)]) for x in et))
