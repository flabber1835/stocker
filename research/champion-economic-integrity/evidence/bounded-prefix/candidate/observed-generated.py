...
gc.disable()
ROOT = Path('sharadar')
OUT = Path('/home/runner/work/stocker/stocker/economic-prefix-audit/candidate/engine')
        ...
        for date,g in d.groupby('date',sort=True):
            ...
            if prior_recent_sel:
                ...
                for tid0 in prior_recent_sel:
                    ...
                    _lead_sid=str(sid[int(tid0)]); _lead_event=_lead_events.get(_lead_sid)
                    ...
                    _lead_ret,_lead_source=_closure.production_leadership_return(_lead_return,audit=_PATH,security_id=_lead_sid,session=ds,prior_signal=p0,current_signal=p1,prior_raw=_lead_prior_raw.get(int(tid0)),terminal=_lead_event,delivered_raw=_delivered_px,split_ratio=canonicalsplit[int(tid0)])
            ...
            _lead_prior_raw={int(t):float(clraw[int(t)]) for t in recsel if finite(clraw[int(t)])}
            ...
            _economic_audit_hook('before_actions',locals())
            ...
            _economic_audit_hook('after_splits',locals())
            ...
            _economic_audit_hook('before_fills',locals())
            for s in book.slots:
                ...
                if finite(px) and px>0 and finite(volume[s.tid]) and volume[s.tid]>0:
                    ...
                    s.tid=-1; s.qty=0.; s.entry_sig=np.nan; s.peak=np.nan; s.entry_day=-1; s.reviewed=False; s.pending_sell=False; s.sell_reason=''; s.ready_day=gday+COOLDOWN
            _economic_audit_hook('after_sells',locals())
            for s in book.slots:
            ...
            _economic_audit_hook('after_buys',locals())
            ...
            _economic_audit_hook('after_dividends',locals())
            for tid0 in tids:
            ...
            _economic_audit_hook('close_nav',locals())
            held=[]
            for s in book.slots:
                if not s.held(): continue
            ...
            _PATH.observe_session(
                session=ds,tids=tids,sid=sid,tick=tick,metadata_fn=_metadata,classification_fn=_BEST_PATH_CLASS,
            ...
            _economic_audit_hook('session_end',locals())
        if rows:
