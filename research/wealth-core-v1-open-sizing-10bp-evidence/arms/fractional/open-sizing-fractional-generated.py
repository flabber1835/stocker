        ...
        for date,g in d.groupby('date',sort=True):
            ...
            for s in book.slots:
                ...
                if finite(px) and px>0 and finite(volume[tid]) and volume[tid]>0:
                    _execution_budget=max(0.0,min(_desired,_cash_before)); q=float(_execution_budget/(float(px)*(1+COST))) if _execution_budget>0 else 0.0
                    if q>1e-12:
                        ...
                        open_cash_limited_execs+=int(_cash_limited); open_rounding_underfill_dollars+=_rounding; open_rounding_underfill_fraction_sum+=_round_frac; open_full_target_execs+=int(abs(_gross-_desired)<=max(1e-8,1e-10*_desired)); open_partial_target_execs+=int(abs(_gross-_desired)>max(1e-8,1e-10*_desired)); open_fractional_buy_count+=int(q>0 and abs(q-round(q))>1e-9)
                        open_sizing_event_rows.append({'decision_date':str(_pm.get('decision_date','')),'execution_date':ds,'ticker':str(tick[tid]),'mode':'fractional','close_price':float(_pm.get('close_price',np.nan)),'open_price':float(px),'intended_target_dollars':_desired,'cash_before_execution':_cash_before,'execution_budget':_execution_budget,'executed_shares':float(q),'executed_gross_dollars':_gross,'entry_fraction_of_intended':_frac,'cash_limited_at_open':_cash_limited,'rounding_underfill_dollars':_rounding,'rounding_underfill_fraction':_round_frac,'close_required_10bp_cushion':float(_pm.get('close_required_reserve',np.nan)),'close_cash_above_cushion':float(_pm.get('close_uncommitted_cash',np.nan)),'blocked':False,'block_reason':''})
