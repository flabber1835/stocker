"""Resumable, time-bounded January formation using the unchanged production kernel."""
...
from .supplement import supplemented_terminals
...
def reference():
    path = Path(__file__).with_name("reference-daily.csv.gz")
...
def progress(stats, day, nav, refs):
    if day < START:
        return dict(stats), {"date": day, "phase": "FORMATION", "cagr": None,
                             "reference_cagr": None, "nav": str(nav)}
    result = dict(stats)
    if not result:
        if day != START:
            raise ValueError("measurement baseline is absent")
        result = {"base": str(nav), "peak": str(nav), "drawdown": "0", "sessions": 0}
    result["sessions"] += 1
    value = Decimal(nav)
    ...
    ref_multiple = refs[day]/refs[START]
    years = (date.fromisoformat(day)-date.fromisoformat(START)).days/365.2425
    return result, {"date": day, "phase": "PROVISIONAL_PARTIAL_MEASUREMENT",
                    "nav": str(value), "measurement_base": result["base"],
                    "multiple": str(multiple), "maximum_drawdown": result["drawdown"],
                    "cagr": math.expm1(math.log(float(multiple))/years) if years else None,
                    "reference_multiple": str(ref_multiple),
                    "reference_cagr": math.expm1(math.log(float(ref_multiple))/years) if years else None}
...
def main():
    ...
    manifest = json.loads(Path(__file__).with_name("production-source.json").read_text())
    ...
    binding = {"production": PRODUCTION_REVISION, "production_manifest": digest(manifest),
               ...
               "harness": {n: sha256(Path(__file__).with_name(n)) for n in
    ...
    with (args.output / "daily.jsonl").open("w", encoding="utf-8") as trace:
        for day, rows in data.sessions(after=state.last_processed_session):
            ...
            while time.monotonic() < deadline:
                ...
                total += 1
                stats, status = progress(stats, day, economics["strategy_nav"], refs)
