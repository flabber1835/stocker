...
def dynamic_peer_breadth(held,gday,close_ring,spy,shadow_dates):
    ...
    for i,z in enumerate(held):
        ...
        amber=individual or (stressed and not greens[i])
...
class Native:
    ...
    def step(self,ob):
        ...
        if finite(dd) and dd>-.06: self.base_fast_armed=True
        ...
        if finite(dd) and dd>-.06: self.fast_armed=True
        ...
        target=0. if (self.fast or self.slow) else 1.
...
class CandidateA:
    ...
    def step(self,native,effective_native,wcdd,recent_r20,recent_r40,spy20,wc_r20):
        ...
        if self.previous_native_was_full and native<1-1e-12:
            if not self.episode: self._audit['episodes']+=1
            self.episode=True
    ...
    _schema = 'ramp-free-ex3-rebound-true/1'
