#!/usr/bin/env python3
"""Patch the frozen Caesar experiment harness for liquidity-only traversal validation.

The original final-validation runner incorrectly required liquidity-stressed runs to
visit exactly the same security-classification population as the certified baseline.
A stricter liquidity gate causally removes candidates before classification traversal,
so exact traversal equality is not an invariant of these three experiments.

This patch keeps the frozen classifier/source/data/dividend checks and replaces only
that invalid equality assertion with internal accounting, monotonicity, and binding
checks for LIQ_* arms. Non-liquidity behavior remains byte-for-byte represented by the
original strict branch in the replacement block.
"""
...
def main() -> int:
    ...
    print("PASS liquidity traversal validation patch applied exactly once")
    return 0
