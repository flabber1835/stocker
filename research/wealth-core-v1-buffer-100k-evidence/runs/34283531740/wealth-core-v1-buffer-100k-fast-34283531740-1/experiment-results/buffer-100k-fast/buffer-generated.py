        ...
        for date,g in d.groupby('date',sort=True):
            ...
            for s in book.slots:
                ...
                if finite(px) and px>0 and finite(volume[tid]) and volume[tid]>0:
                    _planned_q=int(round(s.pending_shares)); _buffer_required=max(0.0,float(open_eq)*0.001); _buffer_available=max(0.0,book.cash-_buffer_required); afford=math.floor(_buffer_available/(float(px)*(1+COST))); q=min(_planned_q,afford); buffer_blocked_open_entries+=int(_planned_q>=1 and q<1)
                    if q>=1:
            ...
            if first_eligible is not None and gday>=first_eligible:
                ...
                if ready and not unresolved and book.cash>0:
                    ...
                    for tid0 in durable:
                        ...
                        _desired=float(eq*ENTRY_W); _buffer_required_close=max(0.0,float(eq)*0.001); _buffer_available_close=max(0.0,book.cash-_buffer_required_close); target=min(_desired,_buffer_available_close); _funding_fraction=(float(target)/_desired if _desired>0 else 0.0); q=int(target//(float(px)*(1+COST)))
