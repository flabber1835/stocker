        ...
        for date,g in d.groupby('date',sort=True):
            ...
            for s in book.slots:
                ...
                if finite(px) and px>0 and finite(volume[tid]) and volume[tid]>0:
                    _planned_q=int(round(s.pending_shares)); _buffer_required=max(0.0,float(open_eq)*0.0015); _cash_before=float(book.cash); _buffer_available=max(0.0,_cash_before-_buffer_required); afford=math.floor(_buffer_available/(float(px)*(1+COST))); q=min(_planned_q,afford); _pm=_pending_meta.get(id(s),{}); buffer_blocked_open_entries+=int(_planned_q>=1 and q<1); (_close_px:=float(_pm.get('close_price',np.nan))); (_close_uncommitted:=float(_pm.get('close_uncommitted_cash',np.nan))); (_close_required:=float(_pm.get('close_required_reserve',np.nan))); (_gap_pct:=((float(px)/_close_px-1.0)*100.0 if finite(_close_px) and _close_px>0 else np.nan)); (_planned_close_notional:=float(_planned_q)*_close_px if finite(_close_px) else np.nan); (_executed_open_notional:=float(max(q,0))*float(px)); (_planned_open_gross:=float(_planned_q)*float(px)*(1+COST)); (_close_plan_gross:=float(_planned_q)*_close_px*(1+COST) if finite(_close_px) else np.nan); (_gap_alone_would_clip:=bool(finite(_close_uncommitted) and _close_uncommitted+1e-12<_planned_open_gross)); gap_event_rows.append({'decision_date':str(_pm.get('decision_date','')),'execution_date':ds,'ticker':str(tick[tid]),'intended_shares':int(_planned_q),'executed_shares':int(max(q,0)),'close_price_used_for_planning':_close_px,'next_open_price':float(px),'close_to_open_percentage_gap':_gap_pct,'cash_available_before_execution':_cash_before,'required_reserve':_buffer_required,'uncommitted_cash_after_reserve':_buffer_available,'close_required_reserve':_close_required,'close_uncommitted_cash_after_reserve':_close_uncommitted,'intended_notional':_planned_close_notional,'executed_notional':_executed_open_notional,'became_q0':bool(q<1),'buffer_basis_points':15,'gap_alone_would_clip':_gap_alone_would_clip,'open_uncommitted_cash_change_vs_close':(_buffer_available-_close_uncommitted if finite(_close_uncommitted) else np.nan)}) if q<_planned_q else None
                    if q>=1:
            ...
            if first_eligible is not None and gday>=first_eligible:
                ...
                if ready and not unresolved and book.cash>0:
                    ...
                    for tid0 in durable:
                        ...
                        _desired=float(eq*ENTRY_W); _buffer_required_close=max(0.0,float(eq)*0.0015); _buffer_available_close=max(0.0,float(book.cash)-_buffer_required_close); target=min(_desired,_buffer_available_close); _funding_fraction=(float(target)/_desired if _desired>0 else 0.0); _one_share_gross=float(px)*(1+COST); q=int(target//_one_share_gross)
                        if q<1: scale_q0_candidate_skips+=1; _q0_reason=('CASH_SCARCITY' if _buffer_available_close+1e-12<_one_share_gross else 'TARGET_GRANULARITY'); close_decision_rows.append({'decision_date':ds,'ticker':str(tick[tid]),'outcome':'Q0_SKIP','planned_shares':0,'close_price':float(px),'close_nav':float(eq),'cash_before_decision':float(book.cash),'required_reserve':float(_buffer_required_close),'uncommitted_cash_after_reserve':float(_buffer_available_close),'intended_capital':float(_desired),'funding_fraction':float(_funding_fraction),'q0_reason':_q0_reason,'buffer_basis_points':15}); continue
                        scale_cash_limited_decisions+=int(_funding_fraction<0.999999999); close_decision_rows.append({'decision_date':ds,'ticker':str(tick[tid]),'outcome':'PLAN','planned_shares':int(q),'close_price':float(px),'close_nav':float(eq),'cash_before_decision':float(book.cash),'required_reserve':float(_buffer_required_close),'uncommitted_cash_after_reserve':float(_buffer_available_close),'intended_capital':float(_desired),'funding_fraction':float(_funding_fraction),'q0_reason':'','buffer_basis_points':15})
                        s=ready[ad]; s.pending_tid=tid; s.pending_shares=float(q); s.pending_signal_day=gday; s.pending_intended_capital=_desired; _pending_meta[id(s)]={'decision_date':ds,'close_price':float(px),'close_nav':float(eq),'close_cash_before':float(book.cash),'close_required_reserve':float(_buffer_required_close),'close_uncommitted_cash':float(_buffer_available_close),'planned_shares':int(q),'intended_capital':float(_desired)}; resids.add(tid); resissuers.add(issuer_key(tid,ds)); ad+=1
