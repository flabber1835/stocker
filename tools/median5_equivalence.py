...
class Comparison:
    def __init__(self, dataset, classifier, end, output, restart_every, *,
        ...
        self.champion = champion_enabled(self.identity)
    ...
    def observe(self, research):
        ...
        if self.champion:
            from sentinel.controller.champion import NATIVE_FIELDS
            for reference_name, production_name in NATIVE_FIELDS.items():
                self.equal("native_state_" + reference_name,
                    after.controller[production_name],
                    getattr(research["native"], reference_name),
                    numeric=reference_name == "base_anchor")
            for field in ("episode", "latched", "full_streak", "recent_positive_streak"):
                self.equal("recovery_state_" + field, after.median5[field],
                    getattr(research["ca"], field))
            self.equal("recovery_prior_native", after.median5["previous_native"] == 1.,
                research["ca"].previous_native_was_full)
            self.equal("recovery_audit", after.median5["champion_audit"], research["ca"]._audit)
...
def main(*, profile="median5"):
    ...
    if champion:
        from sentinel.controller.champion_config import load as v5_load
    ...
    if args.through == "2026-07-31":
        ...
        if champion:
            expected_metrics = {"cagr": .22323600023175572,
                                "max_drawdown": -.2652803738241426,
                                "sharpe": 1.154056575089081,
        ...
        comparison.equal("certified_reference_transitions", research_summary["transition_counts"]["A"], 23 if champion else 28 if v5 else 25)
        comparison.equal("certified_reference_zero_allocation_sessions",
                         sum(row["allocation"] == 0 for row in comparison.rows
                             if row["session"] >= "2006-07-31"), 694 if champion else 634 if v5 else 459)
        if v5:
