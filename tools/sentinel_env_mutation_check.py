...
MUTANTS = (
    ...
    *tuple(
    ...
    ("automation-profile-service-accepted", "scripts/sentinel-env.sh",
     ...
     EXECUTION_ENVELOPE + "test_startup_service_allowlist_blocks_profile_activation"),
    ("destructive-down-options-accepted", "scripts/sentinel-env.sh",
     'if command == "down":\n    parse_command_options(tail, flags=set(), values={"-t", "--timeout"})\n    raise SystemExit(0)',
     'if command == "down":\n    parse_command_options(tail, flags={"-v", "--volumes", "--remove-orphans"}, values={"-t", "--timeout"})\n    raise SystemExit(0)',
     EXECUTION_ENVELOPE + "test_destructive_and_future_operational_options_are_refused"),
    ("emergency-compose-runtime-reintroduced", "scripts/sentinel-emergency-kill.sh",
     'docker --context default exec -i "$POSTGRES_ID"',
     'docker --context default compose exec -i "$POSTGRES_ID"',
