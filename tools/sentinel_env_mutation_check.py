...
EMERGENCY_BOUNDARY = "tests.scripts.test_sentinel_emergency_kill.EmergencyKillBoundary."
MUTANTS = (
    ...
    *tuple(
    ...
    ("run-service-selection-accepted", "scripts/sentinel-env.sh",
     "            if service != expected:\n                refuse(\"Compose run service is outside the Sentinel execution envelope: \" + service)",
     "            if False and service != expected:\n                refuse(\"Compose run service is outside the Sentinel execution envelope: \" + service)",
    ...
    ("stale-container-revival-accepted", "scripts/sentinel-env.sh",
     ...
     'if False and command in {"start", "restart", "unpause"}:\n    refuse("stale-container revival is not allowed; use the reviewed Compose up path")',
     EXECUTION_ENVELOPE + "test_stale_container_revival_and_exec_are_refused"),
    ...
    ("automation-profile-service-accepted", "scripts/sentinel-env.sh",
     ...
     '        if False and surface == "automation" and token in forbidden_automation_services:',
     EXECUTION_ENVELOPE + "test_automation_profile_cannot_explicitly_activate_other_profile_services"),
    ...
    ("emergency-compose-runtime-reintroduced", "scripts/sentinel-emergency-kill.sh",
     'printf \'%s\\n\' "$SQL" | docker --context default exec -i "$POSTGRES_ID" \\\',
     'printf \'%s\\n\' "$SQL" | docker --context default compose exec -i "$POSTGRES_ID" \\\',
     EMERGENCY_BOUNDARY + "test_script_has_no_compose_or_sentinel_runtime_dependency"),
    ("emergency-idempotence-removed", "scripts/sentinel-emergency-kill.sh",
     "   WHERE id=1 AND NOT kill_switch_engaged",
     "   WHERE id=1",
     EMERGENCY_BOUNDARY + "test_targets_exact_canonical_postgres_and_direct_sql"),
