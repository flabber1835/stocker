...
def _docker_image_identity(root: Path, reference: str) -> tuple[str, str]:
    ...
    _require_git_object(revision, label="authorized image source revision")
...
def build_input(*, root: Path, commit: str, workflow_run: int,
    ...
    if revision != commit:
        raise CertificationManifestRefused(
            "authorized image revision differs from source commit")
...
def _validate_input(value: Mapping[str, Any]) -> None:
    ...
    if not isinstance(locks, dict) or set(locks) != {
        ...
        raise CertificationManifestRefused(
            "dependency lock hashes are invalid")
    ...
    if not isinstance(counts, dict) or counts.get("suites_completed") != 3:
        raise CertificationManifestRefused(
            "required software test count is invalid")
    ...
    if total.get("passed", 0) <= 0 or any(
        ...
        raise CertificationManifestRefused(
            "software test totals contain non-passes")
    for name in ("adversarial_evidence", "mutation_evidence"):
        row = value.get(name)
        if not isinstance(row, dict) or row.get("status") != "PASS":
            raise CertificationManifestRefused(
                "%s status is not PASS" % name)
    ...
    if value.get("runtime_schema_epoch") != RUNTIME_SCHEMA_EPOCH or \
        ...
        raise CertificationManifestRefused(
            "software certification epoch is unsupported")
...
def finalize_manifest(*, input_evidence: Mapping[str, Any],
    ...
    image_digest = _require_image_digest(
        image_digest, label="registry image digest")
    if not subject_name.startswith("ghcr.io/") or "@" in subject_name:
        raise CertificationManifestRefused("registry subject name is invalid")
    if publication_run <= 0 or publication_attempt <= 0:
        raise CertificationManifestRefused(
            "publication workflow identity is invalid")
    try:
        parsed = datetime.fromisoformat(certified_at.replace("Z", "+00:00"))
    except (AttributeError, ValueError) as exc:
        raise CertificationManifestRefused(
            "certification timestamp is invalid") from exc
    if parsed.tzinfo is None:
        raise CertificationManifestRefused(
            "certification timestamp has no timezone")
    counts = input_evidence["test_counts"]
    jobs = _required_jobs(jobs_payload)
    manifest = {
...
def verify_manifest(value: Mapping[str, Any]) -> None:
    if value.get("schema") != MANIFEST_SCHEMA:
        raise CertificationManifestRefused(
            "certification manifest schema is unsupported")
    ...
    if supplied != sha256_bytes(canonical_bytes(unsigned)):
        raise CertificationManifestRefused(
            "certification manifest integrity check failed")
    source = value.get("source")
    runtime = value.get("runtime")
    tests = value.get("tests")
    ci = value.get("ci")
    if not all(isinstance(item, dict) for item in (source, runtime, tests, ci)):
        raise CertificationManifestRefused(
            "certification binding is incomplete")
    ...
    if source.get("repository") != REPOSITORY:
        raise CertificationManifestRefused(
            "manifest repository authority is invalid")
    digest = _require_image_digest(
        runtime.get("docker_image_digest"), label="manifest image digest")
    if runtime.get("authorized_runtime_digest") != digest or \
            runtime.get("immutable_ref") != "%s@%s" % (
                runtime.get("subject_name"), digest):
        raise CertificationManifestRefused(
            "immutable runtime binding is inconsistent")
    if ci.get("test_workflow_path") != TEST_WORKFLOW_PATH:
        raise CertificationManifestRefused(
            "certification workflow binding is invalid")
    if tests.get("required_job_conclusions") != {
            name: "success" for name in REQUIRED_JOBS}:
        raise CertificationManifestRefused(
            "required CI job conclusions are invalid")
    ...
    if not isinstance(required, dict) or required.get("passed", 0) <= 0 or \
        ...
        raise CertificationManifestRefused(
            "required test counts are invalid")
