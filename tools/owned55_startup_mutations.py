...
STARTUP_CASES = [
    ('formation_env_range_unbounded', 'scripts/sentinel_env.py',
     ...
     'tests/host_python38/test_env_review_fixes.py::EnvReviewFixes::test_every_typed_runtime_family_refuses_malformed_values'),
    ('formation_uses_provider_budget', 'scripts/sentinel_autonomous_deploy.py',
     'deadline = started + self.cfg.formation_timeout_seconds',
     'deadline = started + self.cfg.data_wait_timeout_seconds',
     'tests/sentinel/test_autonomous_deploy_driver.py::test_driver_formation_cannot_consume_provider_wait_budget'),
    ('formation_env_collision', 'scripts/sentinel_autonomous_deploy.py',
     'env.get("SENTINEL_DEPLOY_FORMATION_TIMEOUT_SECONDS", "7200")',
     'env.get("SENTINEL_DEPLOY_DATA_WAIT_TIMEOUT_SECONDS", "7200")',
     'tests/sentinel/test_autonomous_deploy_driver.py::test_shipped_wait_settings_agree_with_actual_driver'),
    ('observation_cold_fallback', 'sentinel/observation_authority.py',
     'if owned(strategy):', 'if False:',
     'tests/sentinel/test_rolling_admission_readers.py::test_warmup_is_the_selected_canonical_production_transition'),
    ('observation_cold_proof_allowed', 'sentinel/observation_startup.py',
     'if owned:', "if owned and warmup.get('schema') != COLD_SCHEMA:",
     'tests/sentinel/test_rolling_admission_readers.py::test_owned_candidate_and_issuer_refuse_rehashed_cold_warmup'),
    ('observation_issuer_guard_removed', 'tools/sentinel_observation_authority.py',
     'observation_startup.require(warmup,', '(lambda *args, **kwargs: None)(warmup,',
     'tests/sentinel/test_rolling_admission_readers.py::test_owned_candidate_and_issuer_refuse_rehashed_cold_warmup'),
    ('go_partition_failure_ignored', 'scripts/sentinel_go_suites.py',
     'complete = False', 'complete = True',
     'tests/scripts/test_sentinel_go_suites.py::test_any_incomplete_partition_blocks_go'),
    ('go_partition_misclassified', 'scripts/sentinel_go_suites.py',
     "if filename.startswith('test_rolling_'):", 'if False:',
     'tests/scripts/test_sentinel_go_suites.py::test_partition_plugin_covers_unknown_and_nested_modules_once'),
    ...
    ('formation_health_timeout', 'scripts/sentinel_autonomous_deploy.py',
     'env.get("SENTINEL_DEPLOY_FORMATION_TIMEOUT_SECONDS", "7200")',
     'env.get("SENTINEL_DEPLOY_FORMATION_TIMEOUT_SECONDS", "300")',
