...
## Million-row lane work and diagnostics

The warmup lane retains its full 300-session, more-than-one-million-row source
seed and production universe floor. Its synthetic HTTP provider must not sort
and canonicalize that entire observation again for every cursor page or
unchanged corroboration request. Index source rows by date lazily, retain at
most one prepared query per table, and reuse its exact canonical row bytes.
Retain compact export descriptors by complete request and active fault set so
monthly preflight and corroboration cycles reuse already-issued bytes without
retaining every partition's prepared row objects. Each request still advances
its observation counter and records current revision state before cache lookup.
Advance and activated source revisions invalidate the affected caches;
cursor-dependent faults participate in the prepared-query key. Issued export
bytes remain immutable across later revisions. Response rows, source
fingerprints, generation identities, ordering and all adversarial activation
semantics must remain identical to the uncached simulator. This is test-only
transport acceleration, not a production source-authority exemption.

The lane must stream captured Python output, name each executing test, and
report the source-seed and canonical warmup phases with their session/row
counts and elapsed times. The canonical warmup counter explicitly names its
security-feature-series unit, rather than implying it counts source bars.
The completed source-seed counter sums parsed SEP rows from acquired-file
receipts and matches the fixture's source size; the final publication's repair
batch is not the total acquired seed.
A periodic Python-frame diagnostic identifies code
that stalls between progress events without asynchronous native stack dumping.
Do not infer completion percentages or ETAs from elapsed time. The existing
job deadline and mandatory evidence gate remain unchanged; a timeout still
cannot produce certification.
