...
4. After all software checks pass on `main`, CI creates `software-certification-input.json` containing:
...
5. The protected publication workflow downloads the exact tested-image artifact and verifies its checksum manifest.
6. It queries the originating safety run and requires both `host-python-38-exact-head` and `sentinel-exact-head` to have conclusion `success`.
7. It computes the final OCI registry digest through the existing pinned loopback-registry publication path.
8. `certification.json` binds the source, tree, dependency locks, tests, required job conclusions, evidence hashes, workflow IDs, epochs, timestamp, exact GHCR digest, and immutable `name@digest` reference.
9. The certification manifest carries `manifest_sha256`, computed over canonical JSON excluding only that field. Verification fails on any content change.
10. Existing Sigstore SLSA provenance remains required and is bound to the same image digest.
11. The image is copied to GHCR and re-resolved to prove the destination digest is unchanged.
12. The complete certification/provenance/checksum bundle is attached to `ghcr.io/flabber1835/stocker/sentinel-authorized@sha256:…` as an OCI referrer with artifact type `application/vnd.stocker.sentinel.software-certification.v1`.
13. Publication fails if that exact certification referrer cannot be rediscovered from the exact runtime digest.
...
- wrong source commit/tree refuses manifest construction
- missing dependency locks refuses manifest construction
...
Run the unit contract locally in the project test environment:
