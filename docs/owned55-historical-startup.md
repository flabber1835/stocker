...
| CI and mutations | Relevant regressions, meaningful guard-removal faults, syntax and ownership checks | Thirty-eight mutations killed (eight controller/candidate, five integration/accounting, six deployment timing/configuration, two price domains, three observation, twelve audit-only, two authenticated GO connection faults). Ordinary CI and all 6,548 local-full tests passed at 54cd0a24; both image proofs and readiness passed, then GO refused a rolling-health password loss. The real authenticated payload now passes locally; replacement-head CI and complete GO remain gates |
