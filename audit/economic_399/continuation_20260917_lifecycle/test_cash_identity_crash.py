...
def test_same_numeric_position_spelling_renews_grace(conn,pg):
    p=setup_plan(conn)
    for seconds,spelling in [(0,'1'),(119,'1.0'),(238,'1.00'),(357,'1.000')]:
        c=feed_store.connect(pg.sync_dsn)
        try:
            ob=observation(spelling,T0+timedelta(seconds=seconds))
            assert ob.positions[0].quantity==D(1)
            with pytest.raises(PaperRetryableRefused):
                check(c,p,T0+timedelta(seconds=seconds),qty=spelling)
            c.rollback()
        finally: c.close()
    assert count(conn)==4
    with pytest.raises(PaperActivationRefused) as caught:
        check(conn,p,T0+timedelta(seconds=478),qty='1.000')
    assert not isinstance(caught.value,PaperRetryableRefused)
    assert journal.load_commands(conn,DEP)[0].filled_quantity==D(1)
    print({'equal_position_encodings':['1','1.0','1.00','1.000'],
           'last_retry_seconds_since_same_mismatch':357,'durable_grace_cursors':count(conn)})
