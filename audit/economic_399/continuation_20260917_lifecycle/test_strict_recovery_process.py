...
from sentinel import binding as B
...
def test_fresh_process_strict_policy(conn,pg,mode):
    env=dict(os.environ,SENTINEL_RECOVERED_ORDER_AUTHORITY='STRICT_V1')
    done=subprocess.run([sys.executable,'-c',SCRIPT,pg.sync_dsn,mode],env=env,
